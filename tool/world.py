...
async def world(db, cursor):
    ...
    print("<<------OK PROCESSING------>>")
    base_url = 'https://www.channelstv.com/category/world-news/'
            ...
            if li:
                ...
                if link and link != '#':
                    ...
                    cursor.execute("INSERT INTO articles (title, content, category, image, date_published) VALUES (?, ?, ?, ?, ?)",
                                   (title_text, content_text, 'world', image, None))  # Here, date_published is None since it's not provided in the scraping
